...
# ----------------------------------------------------------------------------
# STEP 1: HOOK TO ADD COMMAND LINE OPTIONS
# ----------------------------------------------------------------------------
def pytest_addoption(parser):
    ...
    """
    parser.addoption("--ui_browser", default="chromium", help="Browser: chromium, firefox, webkit")
    parser.addoption("--ui_headed", action="store_true", help="Run in headed (visible) mode")
    parser.addoption("--base_url", default="https://tutorialsninja.com/demo/", help="Base URL for tests")
    parser.addoption("--ui_video", default="retain-on-failure", help="Record video: on, off, retain-on-failure")
    parser.addoption("--ui_screenshot", default="only-on-failure", help="Take screenshot: on, off, only-on-failure")
    parser.addoption("--ui_tracing", default="retain-on-failure", help="Tracing: on, off, retain-on-failure")
...
def browser_context(request):
    ...
    browser_name = get_config_value(request.config, "--ui_browser")
    headed_flag = get_config_value(request.config, "--ui_headed")
    video_option = get_config_value(request.config, "--ui_video")
    ...
    #     browser = playwright.chromium.launch(headless=not headed_flag)  # headless= False
    ...
    # Normalize browser_name (list → string)
...
def page(request, browser_context):
    ...
    screenshot_option = get_config_value(request.config, "--ui_screenshot")
    tracing_option = get_config_value(request.config, "--ui_tracing")
    video_option = get_config_value(request.config, "--ui_video")
